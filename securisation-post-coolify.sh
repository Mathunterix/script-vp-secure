...
# Persister les regles DOCKER-USER via systemd
# (pas iptables-persistent qui peut desinstaller UFW)
DOCKER_USER_SCRIPT="/usr/local/sbin/docker-user-rules.sh"

cat > "$DOCKER_USER_SCRIPT" <<RULES
#!/bin/bash
# Generated by securisation-post-coolify.sh ($TIMESTAMP)
# Re-applique les regles DOCKER-USER apres demarrage de Docker

# Attendre que la chaine DOCKER-USER existe (max 30s)
...
# Ports base de donnees (bloquer depuis Internet uniquement)
for port in 5432 6543; do
    iptables -C DOCKER-USER -i $MAIN_IFACE -p tcp --dport "\$port" -j DROP 2>/dev/null || \
        iptables -I DOCKER-USER -i $MAIN_IFACE -p tcp --dport "\$port" -j DROP
done
RULES
fi
...
cat > /etc/systemd/system/docker-user-rules.service <<'SERVICE'
[Unit]
...
systemctl enable docker-user-rules.service >/dev/null 2>&1
ok "Regles DOCKER-USER persistees (service systemd)"
...
do_rollback() {
    # Supprimer le service systemd si cree
    systemctl disable docker-user-rules.service 2>/dev/null || true
    rm -f /etc/systemd/system/docker-user-rules.service 2>/dev/null || true
    rm -f /usr/local/sbin/docker-user-rules.sh 2>/dev/null || true
    systemctl daemon-reload 2>/dev/null || true

    # Restaurer iptables (avant UFW, pour eviter les conflits)
    ...
    # Verifier que UFW est bien actif apres rollback
    if ! ufw status 2>/dev/null | grep -q "Status: active"; then
        warn "UFW ne semble pas actif apres rollback. Reinstallation..."
